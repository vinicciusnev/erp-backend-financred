...
CREATE TABLE financred.historico_pagamentos (
    id serial PRIMARY KEY,
    emprestimo_id int8 NOT NULL,
    parcela_id int8 NULL,
    ...
    created_at timestamp DEFAULT now(),
    updated_at timestamp DEFAULT now(),
    CONSTRAINT historico_pagamentos_emprestimo_id_fkey
        FOREIGN KEY (emprestimo_id) REFERENCES financred.emprestimo(id) ON DELETE CASCADE,
    CONSTRAINT historico_pagamentos_parcela_id_fkey
        FOREIGN KEY (parcela_id) REFERENCES financred.emprestimo_parcelas(id) ON DELETE SET NULL
);
